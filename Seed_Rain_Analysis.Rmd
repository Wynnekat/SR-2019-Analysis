...
abund.jitter.plot <- ggplot()+
  geom_jitter(data=Uni_Seed, aes(x=Plot, y=totseeds, color = Plot), show.legend = FALSE, width = .2, size = 2) +
    geom_point(data = neg.binom.est, aes(x=site, y=response), show.legend = FALSE, size = 2.5)+
...
**Figure 1**: The number of seeds captured per transect in the remnant prairie and the young (2-year-old), middle-aged (6-7-year-old), and old (15-year-old) reconstructed prairies between May 31st and December 12th, 2019. Asterisk symbol indicates sites that had significantly different amounts of seeds captured when compared to the remnant (p \< 0.05).

### Analysis 1: Results

The quantity of seed rain in reconstructions decreased with age. By at least 6-7 years post-initial seeding, the number of seeds falling in reconstructions was comparable to the remnant prairie in 2019. Only the youngest reconstruction had significantly more seeds falling than the remnant prairie sampled in the same year.
...
NMDS_plot <- ggplot(scrs) +
  ...
  annotate("text", x = -.90, y = 1.1, label = "bold(Stress) == 0.179",
  parse = TRUE)+
...
pairwise.adonis2(data.dist.tot~as.factor(site), data=seed_com)
```
...
Species composition of the seed rain significantly differed between all sites. Reconstructed prairies of comparable ages shared the most similarities in regards to species composition. Except for some marginal overlap with the oldest reconstruction, the remnant prairie contained a distinct community of dispersing plants compared to reconstructed sites. In general, NMDS1 appears to correspond to plant life history traits, where influential annual/biannual species in the seed rain align on the left side of the axis and perennial species on the right side. NMDS2 may be associated with plant functional group, where influential forbs cluster at higher values and graminoids at lower values along the axis.
...
*Note* I later plan to include analyses and figs for c values and Shannon diversity.
...
# Full join species data frame so native species can be filtered 

# For looking at species richness over time

Lumped_Seeds_SPP <- full_join(Lumped_Seeds_SPP, Species_data)

# For looking at total species richness
Seed_Data_Lumped <- full_join(Seed_Data_Lumped, Species_data)
...
Uni_Seed_Diversity$Plot <- factor(Uni_Seed$Plot, levels=c("TP", "PFCA 1", "PFCA 2", "PFCA 3"))
str(Uni_Seed$Plot)
...
response.sr <- c(31.2, 36.5, 41.8, 27.6)
lower.sr <- c(27.9, 32.9, 38.0, 24.5)
upper.sr <- c(34.9, 40.4, 46.0, 31.1)
site <- c("TP", "PFCA 1", "PFCA 2", "PFCA 3")
...
Poisson_sr_native <- glm(Species_richness ~ Plot, family = "poisson", data = Native_Uni_Seed_Diversity)
...
response.sr.native <- c(27, 28, 33, 24)
lower.sr.native <- c(24, 25.0, 30, 21)
upper.sr.native <- c(30, 32, 37, 27)
site <- c("TP", "PFCA 1", "PFCA 2", "PFCA 3")
...
SR.plot <- ggplot(data=Uni_Seed_Diversity)+
  ...
  geom_point(data = Poisson.sr.est, aes(x=site, y=response.sr), show.legend = FALSE, size = 2.5)+
...
SR.plot +  annotate("text", x = 2 , y = 55, label = "*", size = 9.5)+
  geom_vline(xintercept = 3.5, linetype = "longdash")


```

**Figure 4**: The morphospecies richness of seed rain captured per transect in the remnant prairie and the young (2-year-old), middle-aged (6-7-year-old), and old (15-year-old) reconstructed prairies between May 31st and December 12th, 2019. Sites sharing the same letter did not have significantly different amounts of seeds captured from the remnant (p \< 0.05).
...
Native.SR.plot <- ggplot()+
  geom_jitter(data=Native_Uni_Seed_Diversity, aes(x=Plot, y=Species_richness, color = Plot), show.legend = FALSE, width = .2, size = 2) +
  geom_point(data = Poisson.sr.est.native, aes(x=site, y=response.sr.native), show.legend = FALSE, size = 2.5)+
...
Native.SR.plot +  annotate("text", x = 2 , y = 45, label = "*", size = 9.5)+
  geom_vline(xintercept = 3.5, linetype = "longdash")
...
#### Total Species Richness Over Time
...
**Figure 5**: Total morphospecies richness of identified seeds for reconstructed and remnant tallgrass prairies during the study period (May 31 to December 12th, 2019).
...
# setwd("~/Desktop/Seed Rain 2019/Graphics")
Tot_SR_fig <- ggplot(native_week_species_counts, aes(y=Species_richness, x=`Sampling Date`, group=Plot)) +
  geom_line(aes(colour=Plot), size = 1.5) +
  geom_point(aes(shape=Plot, fill=Plot), size=3)+
  scale_color_manual(name = "", values=c("#1f78b4", "#a6cee3", "#b2df8a", "#33a02c"), breaks=c("PFCA 1", "PFCA 2", "PFCA 3", "TP"), labels = c("Young", "Middle", "Old", "Remnant 2019"))+
    scale_fill_manual(name = "", values=c("#1f78b4", "#a6cee3", "#b2df8a", "#33a02c"), breaks=c("PFCA 1", "PFCA 2", "PFCA 3", "TP"), labels = c("Young", "Middle", "Old", "Remnant 2019"))+
...
**Results :** Morphospecies richness was only significantly greater in the middle-aged prairie (6-7-year-old prairie) compared to the remnant. Overall, morphospecies richness trended upward during the study period, with more morphospecies captured in the late fall than in the summer.


## Analysis 5: Supplemental table to assist in cross study comparison

```{r}
Lumped_unknown1 <- Lumped_Seeds_SPP %>% 
  filter(grepl('UNK',SPP6)) %>% 
  group_by(Plot, `Sampling Date`) %>% 
                        summarize(totseeds = sum(totseeds))

Lumped_unknown2 <- Lumped_Seeds_SPP %>% 
...
Lumped_no_unknown <- Lumped_Seeds_SPP %>% 
   filter(!grepl('UNK',SPP6))
