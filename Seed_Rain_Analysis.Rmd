...
#### Community Weighted Mean (CWM) seed mass analysis 
...
#### Seed rain density vs. seed mass (mg) [log 10 scale]
...
ggplot(All_Captured_Sum, aes(x = Mean_1_Seed_Mass_mg, y = Seed_Rain_Density, color = Plot)) +
  geom_point()+
  scale_x_continuous(trans='log10') +
  geom_smooth(method = "glm.nb", se = FALSE) +
   labs(x = "Seed mass (mg) [log scale]", y = "Seed rain density")+
  theme_classic()
...
# Fit a negative binomial model predicting number of seeds by site
mod.nb <- MASS::glm.nb(Seed_Rain_Density~log10(Mean_1_Seed_Mass_mg)*Plot, data=All_Captured_Sum)
...
All_Captured_Sum_Lumped <- All_Captured_Sum %>% 
  group_by(SPP6, Mean_1_Seed_Mass_g) %>% 
  summarize(totseeds = sum(totseeds))


All_Captured_Sum_Lumped$Density <- All_Captured_Sum_Lumped$totseeds * .5
```
...
options(scipen = 999)
# Seed rain density vs. seed mass (mg)
## Density = sum of all seeds captured for a species divided by total trapping area (2 m^2) 
ggplot(All_Captured_Sum_Lumped, aes(x = Mean_1_Seed_Mass_g*1000, y = Density)) +
  geom_point()+
  scale_x_continuous(trans='log10') +
  labs(x = "Seed mass (mg) [log 10 scale]", y = "Seeds"~m^-2)+
  geom_smooth(method = "glm", method.args = list(family = Gamma(link = "log")))+
  theme_classic() +
  theme(text=element_text(size=16), legend.key.size=unit(0.25, "cm"))


#mod.normal <- glm(log(Density) ~ log(Mean_1_Seed_Mass_mg), family = gaussian, data = All_Captured_Sum_Lumped)

#mod.gamma.inv <- glm(Density~ log(Mean_1_Seed_Mass_mg), family = Gamma(link = inverse),  data = All_Captured_Sum_Lumped)


mod.gamma.log <- glm(Density~ log10(Mean_1_Seed_Mass_g*1000), family = Gamma(link = log),  data = All_Captured_Sum_Lumped)

summary(mod.gamma.log)


### Gamma mod with log link is best

#summary(mod.normal)$deviance
#summary(mod.gamma.inv)$deviance
#summary(mod.gamma.log)$deviance
...
All_Captured$Seed_Biomass_g <- All_Captured$Mean_1_Seed_Mass_g * All_Captured$totseeds

# Add all the seed biomass together for each site transect pair
All_Captured_Biomass <- All_Captured %>% 
  group_by(Plot, Transect) %>% 
  summarize(Seed_Biomass_g = sum(Seed_Biomass_g))
...
mod.gamma.biomass <- glm(Seed_Biomass_g~Plot, family = Gamma(link = "log"), data=All_Captured_Biomass)
...
response <- c(1.112, 4.371, 1.378, .882)
lower <- c(.844, 3.320, 1.046,  0.670)
upper <- c(1.46, 5.76, 1.81, 1.16)
site <- c("TP", "PFCA 1", "PFCA 2", "PFCA 3")
...
ggplot() +
  geom_jitter(data=All_Captured_Biomass, aes(x=Plot, y=Seed_Biomass_g, color = Plot), show.legend = FALSE, width = .2, size = 2) +
  ...
  xlab("Site")+
  ylim(0,8)+
  ylab("Seed rain biomass (g)")+
  scale_x_discrete(labels = c ("PFCA 1" = "Young", "PFCA 2" = "Middle", "PFCA 3" = "Old", "TP" = "Remnant"), guide = guide_axis(angle = 0)) +
  scale_color_manual(name = "Site", values=c( "#1f78b4", "#a6cee3", "#b2df8a","#33a02c"), breaks=c("PFCA 1", "PFCA 2", "PFCA 3", "TP"), labels = c("Young", "Middle", "Old", "Remnant"))+
   scale_x_discrete(labels = c ("PFCA 1" = "Young", "PFCA 2" = "Middle", "PFCA 3" = "Old", "TP" = "Remnant"), guide = guide_axis(angle = 0)) +
 theme(text=element_text(size=16), legend.key.size=unit(1, "cm"))+ geom_vline(xintercept = 3.5, linetype = "longdash") + annotate("text", x = 1 , y = 7.5, label = "*", size = 9.5)
```
