...
abund.plot <- ggplot(data=Uni_Seed)+
  geom_boxplot(aes(x=Plot, y=totseeds, fill = Plot), show.legend = FALSE) +
  theme_classic()+
  xlab("Site")+
  ylim(0,10000)+
  ylab("Number of seeds")+
  scale_x_discrete(labels = c ("PFCA 1" = "Young", "PFCA 2" = "Middle", "PFCA 3" = "Old", "TP" = "Remnant"), guide = guide_axis(angle = 0)) +
  scale_fill_manual(name = "Site", values=c( "#1f78b4", "#a6cee3", "#b2df8a","#33a02c"), breaks=c("PFCA 1", "PFCA 2", "PFCA 3", "TP"), labels = c("Young", "Middle", "Old", "Remnant"))+
 theme(text=element_text(size=16), legend.key.size=unit(1, "cm"))+ geom_vline(xintercept = 3.5, linetype = "longdash")
abund.plot + annotate("text", x = 1 , y = 9500, label = "*", size = 9)
```
...
ggplot(New_Seed_Rain_Date, aes(y=Density_J, x=Date, group=Plot)) +
  ...
  theme_classic()+
  scale_x_datetime(date_breaks = '1 month', date_labels="%b %d", limits=as.POSIXct(c("2019-06-10", "2020-1-01")))+
...
NMDS_plot <- ggplot(scrs) +
  ...
  annotate("text", x = -.95, y = 1.1, label = "bold(Stress) == 0.179",
  parse = TRUE)+
  annotate("text", x = -1.12, y = 1, label = "bold(K) == 2",
  parse = TRUE) +
  theme_classic() +
...
# Count the number of distinct species per site according to unique transect 
...
# Count the number of distinct species per site according to sampling date
week_species_counts <- Lumped_Seeds_SPP  %>% 
                  group_by(Plot, `Sampling Date`) %>% 
                  summarize(Species_richness = n_distinct(SPP6))
```

### Analysis 4: Fitting the model
...
# Decided to use the Poisson (dispersion parameter ~ 0.95)
...
# Contrasts
standardize <- lsmeans(Poisson_sr, "Plot")
contrast(standardize, "trt.vs.ctrl")
...
SR.plot <- ggplot(data=Uni_Seed_Diversity)+
  geom_boxplot(aes(x=Plot, y=Species_richness, fill = Plot), show.legend = FALSE) +
  theme_classic()+
  xlab("Site")+
  ylim(0,60)+
...
SR.plot + annotate("text", x = c(1,3,4) , y = 45, label = "B", size = 6) + annotate("text", x = 2 , y = 55, label = "A", size = 6)
...
test<- seed_community_phenology_lumped %>% 
