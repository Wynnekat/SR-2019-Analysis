...
Species_data <- read_excel("SR_SB_VEG_Traits_Dataset.xlsx")
```
...
There was no significant change in CWM seed mass between sites, suggesting that differences in the quantity of seed rain is not related to changes in seed mass traits as community assembly proceeds and restored prairie communities mature. Therefore, the marked increase in seed rain abundance in younger communities is not because there are more small-seeded species dispersing in that community.  


```{r, message = FALSE, echo = FALSE, warning = FALSE}

## Old code
### I'm keeping this code just in case I need it. I want to use the full data though since there are only 4 morphospecies I'm missing seed mass data for: Oenothera filliformis, Rubus spp., Eleocharis spp and Eleocharis spp1. I plan on weighing OENFIL and RUBSPP since we have enough seeds (> 20) in the reference collection and captured seed rain. I will only omit the two Eleocharis spp. morphospecies since there is 1) a paucity of published data for the Eleocharis species in the area and 2) we only caught one seed for each morphospecies. 


### ----

# Only with most frequently captured species

#Most_Frequently_Captured <- Seed_Data_Lumped %>% 
  #group_by(SPP6) %>% 
  #summarize(totseeds = sum(totseeds)) %>% 
  #filter(totseeds >= 50)

#Most_Frequently_Captured <- Most_Frequently_Captured[,-2]


#Most_Frequently_Captured_data <- left_join(Most_Frequently_Captured, 
#Seed_Data_Lumped, multiple = "all")

# Only use the most common species (> 50 seeds captured over the course of the study = top 60 most frequently caught species)


#test <- Most_Frequently_Captured_data %>% 
  #group_by(Plot, Transect) %>% 
  #summarize(weighted_seed_mass = weighted.mean(Mean_1_Seed_Mass_g, totseeds))
...
# Filter out the morphospecies we do not have seed mass data for
...
#### Analysis 5: Seed rain density vs. seed mass (mg) [log 10 scale]


Far more seeds are dispersing in the seed rain of newly restored tallgrass prairies compared to older restorations and remnant communities.  But is this because there are more small-seeded species present in younger communities compared to older prairie communities? It is well known that species that produce small-seeds often produce greater quantities of seed rain than large-seeded species (MANY CITATIONS). Large species that take longer to reach reproductive maturity (like many prairie perennials) also tend to produce large seeds while species that reproduce quickly make small seeds (Moles et al. 2004). 


*1) Is there a relationship between the seed rain and seed mass?*

There is a significant negative relationship between seed rain density and seed mass, where we capture small-seeded species more frequently than species that produce large, heavy seeds. 

*2) Does this relationship change along a chronosequence of restored prairies?*

*3) Is this relationship different for restored vs. remnant prairies?*

As expected from analysis #1, younger restorations have denser seed rain than older restored and remnant prairies. 

However, there was no significant interaction between site and seed mass, suggesting that all our communities had a similar relationship between seed rain and seed mass. 

```{r, message = FALSE, echo = FALSE, warning = FALSE}

All_Captured_Sum <- All_Captured %>% 
  group_by(Plot, SPP6, Mean_1_Seed_Mass_g) %>% 
  summarize(totseeds = sum(totseeds))

All_Captured_Sum$Mean_1_Seed_Mass_mg <- All_Captured_Sum$Mean_1_Seed_Mass_g * 1000

All_Captured_Sum$Seed_Rain_Density <- All_Captured_Sum$totseeds / 0.5
```
...
seedmass.quantity.plot <- ggplot(All_Captured_Sum, aes(x = Mean_1_Seed_Mass_mg, y = totseeds)) +
  geom_point()+
  scale_x_continuous(trans='log10') +
  geom_smooth(method = "glm.nb") +
   labs(x = "Seed mass (mg) [log 10 scale]", y = "Number of seeds")+
  theme_classic() +
...
#mod.gaussian <- glm(log(Seed_Rain_Density) ~ log(Mean_1_Seed_Mass_mg)*Plot, family = gaussian, data=All_Captured_Sum)

# Fit a negative binomial model predicting number of seeds by site
mod.nb <- MASS::glm.nb(totseeds ~log10(Mean_1_Seed_Mass_mg)+log10(Mean_1_Seed_Mass_mg):Plot, data=All_Captured_Sum)

# Convert back to glm() style object  (look into this again you may not need to do it)
mod.nb <- MASS::glm.convert(mod.nb)
...
### Negative binomial model is FAR better

#summary(mod.gaussian)$deviance
#summary(mod.nb)$deviance
...
Because all sites had a similar relationship between seed rain density and seed mass, I combined all the data and reran the analysis. As before, there is a significant negative relationship between seed rain quantity and seed mass (i.e., small-seeded species were captured more frequently on average than large-seed species). 


Overall, we found that species with smaller seeds produced significantly more seeds than large-seeded species (seeds per m2), suggesting a tradeoff between seed production/dispersal and seed size. Seed size is suggested as having a higher survival rate than small-seeded species to offset the numerical advantage afford to small-seeded species (Moles & Westoby, 2006).
...
When considering the total reproductive biomass of the seed rain at each site, the youngest restored community also had the greatest amount of dispersed biomass. Investment in seed rain biomass may be better more advantageous in early assembling communities. 
...
## Analysis 6: Supplemental table to assist in cross study comparison
